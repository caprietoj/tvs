...
#systemsSurveyTable_wrapper .row {
    margin: 0;
}
...
$(document).ready(function() {
    ...
    var table = $('#systemsSurveyTable');
    if (table.length === 0) {
        console.error('Tabla #systemsSurveyTable no encontrada');
        return;
    }
    ...
    $('#systemsSurveyTable').on('mouseenter', 'td', function() {
        var $this = $(this);
            ...
            <div class="card">
                <div class="card-header bg-info text-white">
                    <h3 class="card-title mb-0">
                        ...
                        Respuestas Detalladas del Cuestionario de Sistemas
                    </h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-tool text-white" data-card-widget="collapse" title="Colapsar">
                            <i class="fas fa-minus"></i>
                ...
                <div class="card-body">
                    @php
                        ...
                        if (!function_exists('cleanSystemsText')) {
                            function cleanSystemsText($text) {
                        ...
                        if (!function_exists('cleanSystemsComment')) {
                            function cleanSystemsComment($text) {
                                if (empty($text) || $text === null) {
                    ...
                    @if(!empty($responses) && $responses->count() > 0)
                        <div class="table-responsive">
                            <table id="systemsSurveyTable" class="table table-bordered table-striped table-hover">
                                <thead class="bg-light">
                                    <tr>
                                        <th style="width: 80px;">Fecha</th>
                                        <th style="width: 100px;">Dependencia</th>
                                        <th style="width: 120px;">Tiempos Respuesta</th>
                                        <th style="width: 120px;">Efectividad Técnica</th>
                                        <th style="width: 120px;">Profesionalismo</th>
                                        <th style="width: 120px;">Estado Equipos</th>
                                        <th style="width: 120px;">Apoyo Usabilidad</th>
                                        <th style="width: 120px;">Calidad Internet</th>
                                        <th style="width: 120px;">Intervención Eventos</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($responses as $response)
                                    <tr>
                                        <td>{{ $response->response_timestamp ? $response->response_timestamp->format('d/m/Y') : 'N/A' }}</td>
                                        <td>{{ cleanSystemsText($response->dependencia) }}</td>
                                        <td>{{ cleanSystemsText($response->tiempos_respuesta) }}</td>
                                        <td>{{ cleanSystemsText($response->efectividad_tecnica) }}</td>
                                        <td>{{ cleanSystemsText($response->profesionalismo) }}</td>
                                        <td>{{ cleanSystemsText($response->estado_equipos) }}</td>
                                        <td>{{ cleanSystemsText($response->apoyo_usabilidad) }}</td>
                                        <td>{{ cleanSystemsText($response->calidad_internet) }}</td>
                                        <td>{{ cleanSystemsText($response->intervencion_eventos) }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="text-center text-muted">
                            ...
                            <a href="{{ route('surveys.internal-client.systems.upload') }}" class="btn btn-info">
