---
layout: default
---

# 📚 Documentación del Sistema TVS

Bienvenido a la documentación oficial del Sistema de Gestión TVS (The Victoria School).

## 🚀 Inicio Rápido

### Para Desarrolladores
1. [Instalación](README.md#-instalación)
2. [Configuración](README.md#️-configuración)
3. [Guía de Contribución](CONTRIBUTING.md)

### Para Usuarios
1. [Módulos del Sistema](README.md#-módulos-del-sistema)
2. [Características Principales](README.md#-características-principales)
3. Tutoriales en video (próximamente)

## 📖 Documentación

### General
- [README Principal](README.md) - Visión general del sistema
- [Changelog](CHANGELOG.md) - Historial de cambios
- [Licencia](LICENSE) - Términos de uso

### Técnica
- [Documentación de API](API.md) - Endpoints y ejemplos
- [Guía de Contribución](CONTRIBUTING.md) - Cómo contribuir
- [Estructura del Proyecto](README.md#-estructura-del-proyecto)

### Por Módulo

#### 🏥 Enfermería
- [Sistema de Reconocimiento de Voz](docs/voice-recognition.md)
- [Filtros y Exportación Excel](docs/reports-filtering.md)
- [Gestión de Ingresos](docs/enfermeria-management.md)

#### 🎯 Préstamos de Equipos
- [Restricciones de Fecha](docs/equipment-restrictions.md)
- [Sistema de Notificaciones](docs/notifications.md)

#### 🛒 Compras y Servicios
- [Solicitudes de Compra](CONTEXTO_MODULO_COMPRAS.md)
- [Control Presupuestal](CONFIGURACION_PRESUPUESTO.md)

#### 👥 Colaboradores
- [Gestión de Empleados](MODULO_COLABORADORES_README.md)
- [Importación Masiva](docs/import-colaboradores.md)

#### 🚪 Portería
- [Sistema Completo](Contextos/porteria-sistema-completo.md)
- [Gestión de Visitantes](Contextos/porteria-modal-visitantes.md)

## 🔧 Configuración

### Configuraciones Especiales
- [Google OAuth](CONFIGURACION_GOOGLE_OAUTH.md)
- [Email y Notificaciones](docs/email-config.md)
- [Permisos y Roles](docs/permissions.md)

## 🚀 Despliegue

### Migración y Producción
- [Checklist de Migración](CHECKLIST_MIGRACION.md)
- [Migración a Producción](MIGRACION_PRODUCCION.md)
- [Despliegue Final](DESPLIEGUE_PRODUCCION_FINAL.md)

### Solución de Problemas
- [FAQ Migraciones](FAQ_MIGRACIONES.md)
- [Errores Comunes](docs/troubleshooting.md)
- [Solución Error Migración](SOLUCION_ERROR_MIGRACION_PRODUCCION.md)

## 🎓 Tutoriales

### Básicos
1. [Instalación desde Cero](docs/tutorial-installation.md)
2. [Primer Login y Configuración](docs/tutorial-first-login.md)
3. [Creación de Usuarios](docs/tutorial-users.md)

### Avanzados
1. [Implementar Reconocimiento de Voz](docs/tutorial-voice.md)
2. [Personalizar Reportes Excel](docs/tutorial-excel.md)
3. [Crear Nuevos Módulos](docs/tutorial-modules.md)

## 🌟 Características Destacadas

### Reconocimiento de Voz
El sistema incluye transcripción de voz a texto en formularios de enfermería.

[Ver Documentación Completa →](docs/voice-recognition.md)

### Exportación Excel Profesional
Genera archivos Excel con formato profesional y filtros integrados.

[Ver Documentación Completa →](docs/reports-filtering.md)

### Sistema de Notificaciones
Enrutamiento inteligente de notificaciones por email.

[Ver Documentación Completa →](docs/notifications.md)

## 🤝 Comunidad

### Contribuir
¿Quieres contribuir al proyecto? Lee nuestra [Guía de Contribución](CONTRIBUTING.md).

### Reportar Problemas
Si encuentras un bug, [crea un issue](https://github.com/caprietoj/tvs/issues/new).

### Soporte
- Email: soporte@tvs.edu.co
- GitHub Issues: [Crear Issue](https://github.com/caprietoj/tvs/issues)

## 📊 Estadísticas del Proyecto

![GitHub Stars](https://img.shields.io/github/stars/caprietoj/tvs?style=social)
![GitHub Forks](https://img.shields.io/github/forks/caprietoj/tvs?style=social)
![GitHub Issues](https://img.shields.io/github/issues/caprietoj/tvs)
![GitHub Pull Requests](https://img.shields.io/github/issues-pr/caprietoj/tvs)

## 📝 Últimas Actualizaciones

### Noviembre 2025
- ✅ Reconocimiento de voz en formularios de enfermería
- ✅ Sistema de filtros avanzados en reportes
- ✅ Exportación Excel profesional con SheetJS
- ✅ Restricción de préstamos para el mismo día

[Ver Changelog Completo →](CHANGELOG.md)

## 🗺️ Roadmap

### Q4 2025
- [ ] Integración con sistema de calificaciones
- [ ] Dashboard administrativo mejorado
- [ ] App móvil para enfermería

### Q1 2026
- [ ] Sistema de reportes avanzados
- [ ] Integración con plataforma de pagos
- [ ] Portal de padres de familia

[Ver Roadmap Completo →](https://github.com/caprietoj/tvs/projects)

---

<p align="center">
  <strong>¿Tienes preguntas? <a href="https://github.com/caprietoj/tvs/issues">Pregunta aquí</a></strong>
</p>

<p align="center">
  Desarrollado con ❤️ para The Victoria School (TVS)
</p>
