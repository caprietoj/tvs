...
class SistemasDocumentController extends Controller
{
    ...
    private function storeFolder(Request $request)
    {
        ...
        $basePath = 'public/sistemas/documents/' . $folderName;
    ...
    public function showStructure($id)
    {
        ...
        return view('sistemas.documents.structure', compact('document'));
    }
    ...
    {
        $document = SistemasDocument::with('user')->findOrFail($id);
        
        if ($document->isFile()) {
            // Para archivos individuales, mostrar PDF en viewer
            $filePath = Storage::url($document->file_path);
            return view('sistemas.documents.preview', compact('document', 'filePath'));
        } else {
            // Para carpetas, mostrar estructura de archivos
            $structure = $this->buildFolderStructure($document->file_path);
            return view('sistemas.documents.folder-preview', compact('document', 'structure'));
        }
    }
