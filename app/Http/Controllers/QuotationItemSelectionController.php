...
class QuotationItemSelectionController extends Controller
{
    ...
    {
        try {
            \Log::info('SaveSelection method called', [
                'item_index' => $request->item_index,
                'quotation_id' => $request->quotation_id,
                'unit_price' => $request->unit_price,
            ...
            // Validación paso a paso para mejor debugging
            if (!$request->has('item_index')) {
                throw new \Exception('item_index is missing');
            }
            if (!$request->has('quotation_id')) {
                throw new \Exception('quotation_id is missing');
            }
            if (!$request->has('unit_price')) {
                throw new \Exception('unit_price is missing');
            }

            \Log::info('Basic field validation passed');
            ...
            \Log::info('Laravel validation passed successfully');
            ...
            \Log::info('Item data retrieved', [
                'item' => $item,
                'item_quantity' => $item['quantity'] ?? 'NOT_SET',
                'item_quantity_type' => gettype($item['quantity'] ?? null),
                'request_unit_price' => $request->unit_price,
                'request_unit_price_type' => gettype($request->unit_price)
            ]);

            \Log::info('Starting database transaction');
            DB::beginTransaction();

            // PROTECCIÓN ANTI-DUPLICACIÓN: Verificar si ya existe esta selección exacta
            $existingSelection = QuotationItemSelection::where('purchase_request_id', $purchaseRequest->id)
                ->where('item_index', $request->item_index)
                ->where('quotation_id', $request->quotation_id)
                ->first();

            if ($existingSelection) {
                \Log::warning('Duplicate selection attempt detected, skipping', [
                    'existing_selection_id' => $existingSelection->id,
                    'item_index' => $request->item_index,
                    'quotation_id' => $request->quotation_id
                ]);
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Selección ya existe',
                    'selection' => $existingSelection
                ]);
            }

            // Eliminar selección anterior del mismo item si existe (diferentes proveedores)
            QuotationItemSelection::where('purchase_request_id', $purchaseRequest->id)
                                 ->where('item_index', $request->item_index)
                                 ->delete();

            // Convertir valores a números para evitar errores
            $quantity = is_numeric($item['quantity'] ?? 1) ? (float)($item['quantity'] ?? 1) : 1;
            $unitPrice = is_numeric($request->unit_price) ? (float)$request->unit_price : 0;
            $totalPrice = $quantity * $unitPrice;
            
            \Log::info('Converted values for calculation', [
                'original_quantity' => $item['quantity'] ?? 'NOT_SET',
                'converted_quantity' => $quantity,
                'original_unit_price' => $request->unit_price,
            ...
            \Log::error('Validation error in saveSelection', [
                'errors' => $e->errors(),
            ...
            DB::rollback();
            \Log::error('Error in saveSelection method', [
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
