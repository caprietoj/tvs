...
class PurchaseRequestApproved extends Notification
{
    ...
    private function buildBaseMessage(): MailMessage
    {
        ...
        if ($this->recipient === 'user') {
        ...
        } elseif ($this->recipient === 'compras_fotocopias') {
            $message->subject('Solicitud de Fotocopias Autorizada - #' . $this->purchaseRequest->request_number)
                   ->greeting('Estimado Equipo de Compras')
                   ->line('Se ha **autorizado** una solicitud de fotocopias para su conocimiento.')
                   ->line('**Detalles de la solicitud autorizada:**')
                   ->line('• **Número de solicitud:** ' . ($this->purchaseRequest->request_number ?? 'N/A'))
                   ->line('• **Solicitante:** ' . $this->purchaseRequest->requester)
                   ->line('• **Área/Sección:** ' . $this->purchaseRequest->section_area)
            ...
            // Mostrar información específica de fotocopias
            if ($this->purchaseRequest->copy_items) {
                $copyItems = $this->purchaseRequest->copy_items;
                if (is_string($copyItems)) {
                    $copyItems = json_decode($copyItems, true);
                }
                
                if (is_array($copyItems) && !empty($copyItems)) {
                    $message->line('**Detalles de las fotocopias:**');
                    foreach ($copyItems as $item) {
                        $copies = $item['copies_required'] ?? $item['copies'] ?? 'N/A';
                        $description = $item['original'] ?? $item['description'] ?? 'Sin descripción';
                        $message->line('• ' . $copies . ' copias - ' . $description);
                    }
                }
            }
            
            $message->line('**Nota:** Esta solicitud de fotocopias ha sido enviada al auxiliar de almacén para su preparación.')
                   ->line('No requiere procesamiento de orden de compra.')
                   ->action('Ver Solicitud', url('/purchase-requests/' . $this->purchaseRequest->id))
                   ->salutation('Sistema de Gestión de Compras<br>The Victoria School');
        ...
        } elseif ($this->recipient === 'auxiliaralmacen') {
            $message->subject('Solicitud de Fotocopias Autorizada - #' . $this->purchaseRequest->request_number)
                   ->greeting('Estimado Auxiliar de Almacén')
                   ->line('Se ha **autorizado** una nueva solicitud de fotocopias que requiere su atención.')
                   ->line('**Detalles de la solicitud:**')
                   ->line('• **Número de solicitud:** ' . ($this->purchaseRequest->request_number ?? 'N/A'))
                   ->line('• **Solicitante:** ' . $this->purchaseRequest->requester)
                   ->line('• **Área/Sección:** ' . $this->purchaseRequest->section_area)
            ...
            if ($this->purchaseRequest->copy_items) {
                $copyItems = $this->purchaseRequest->copy_items;
                // Si copy_items es un string JSON, decodificarlo
                if (is_string($copyItems)) {
                    $copyItems = json_decode($copyItems, true);
                }
            ...
            $message->line('Por favor, proceder con la preparación de las fotocopias solicitadas.')
                   ->action('Ver Solicitud', url('/purchase-requests/' . $this->purchaseRequest->id))
                   ->salutation('Sistema de Gestión de Compras<br>The Victoria School');
        }
